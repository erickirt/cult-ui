...
export function GitHubButton({ className }: { className?: string }) {
	return (
		<Link
			...
			rel="noreferrer"
			className={cn(
				"md:flex hidden rounded-md items-center gap-2 border border-border bg-background px-6 py-3 font-mono text-sm uppercase tracking-wider transition-all hover:border-foreground/40 hover:bg-foreground hover:text-background",
				className,
			)}
		>
			<Icons.gitHub className="size-4" />
